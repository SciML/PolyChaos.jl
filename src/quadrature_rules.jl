...
function fejer(N::Int)
    ...
    M = N ÷ 2
    return cos.(θ), map(x -> 2 / N * (1 - 2 * sum(cos(2 * n * x) / (4 * n^2 - 1) for n in 1:M)), θ)
end
...
function fejer2(n::Int)
    ...
    N = 1:2:(n - 1)
    m = n - length(N)
    v0 = push!(push!(map(x -> 2 / x / (x - 2.0), N), 1 / last(N)), zeros(m)...)
...
radau(α::AbstractVector{<:Real}, β::AbstractVector{<:Real}, end0::Real) = radau(length(α) - 2, α, β,
                                                                                end0)
radau(N::Int, op::Union{OrthoPoly,AbstractCanonicalOrthoPoly}, end0::Real) = radau(N, op.α, op.β,
                                                                                   end0)
...
lobatto(α::AbstractVector{<:Real}, β::AbstractVector{<:Real}, endl::Real, endr::Real) = lobatto(length(α) -
                                                                                                3,
                                                                                                α,
                                                                                                β,
                                                                                                endl,
                                                                                                endr)
lobatto(N::Int, op::Union{OrthoPoly,AbstractCanonicalOrthoPoly}, endl::Real, endr::Real) = lobatto(N,
                                                                                                   op.α,
                                                                                                   op.β,
                                                                                                   endl,
                                                                                                   endr)
lobatto(op::Union{OrthoPoly,AbstractCanonicalOrthoPoly}, endl::Real, endr::Real) = lobatto(op.α,
                                                                                           op.β,
                                                                                           endl,
                                                                                           endr)

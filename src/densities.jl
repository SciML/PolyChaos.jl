...
function w_jacobi(t, a, b)
    -1.0 <= t <= 1.0 ? (1 - t)^a * (1 + t)^b : _throwError(t)
end
...
function w_gamma(t, α)
    t >= 0.0 ? (1 / gamma(α) * Float64(t)^(α - 1) * exp(-t)) : _throwError(t)
end

function w_uniform01(t)
    0.0 <= t <= 1.0 ? 1.0 : _throwError(t)
end

function w_uniform_11(t)
    -1.0 <= t <= 1.0 ? 0.5 : _throwError(t)
end

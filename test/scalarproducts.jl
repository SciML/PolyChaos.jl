...
@testset "Norms of orthgonal polynomials ($names)" begin for op in names
    opq = op(deg)
    s = computeSP2(opq)
    ...
    @test isapprox(computeSP(SVector([0, 0, 0, deg - 1, deg - 1]...), opq.α,
                             SVector(opq.β...), opq.quad.nodes,
                             SVector(opq.quad.weights...)), s[deg]; atol = tol)
end end

a, b = 1.1:0.5:4, 1.1:0.5:4
names = [Beta01OrthoPoly, JacobiOrthoPoly, genHermiteOrthoPoly]

@testset "Norms of orthgonal polynomials ($names)" begin for α in a, β in b, name in names

...
@testset "evaluate univariate basis at point(s)" begin
    for x in X
        ...
        [
            @test evaluate(0:deg, x, op) == evaluate(0:deg, x, a, b)
                for (a, b) in combinations
        ]
        [
            @test evaluate(0:2:deg, x, op) == evaluate(0:2:deg, x, a, b)
                for (a, b) in combinations
        ]
        for d in 0:deg
            if typeof(x) <: Real
                ...
                [
                    @test evaluate(d, x, a, b) == evaluate(d, x, op)
                        for (a, b) in combinations
                ]
            end
        end
        ...
        if typeof(x) <: Real
            [
                @test evaluate(x, op) == evaluate([x], op) ==
                    evaluate(collect(0:(op.deg)), [x], op) ==
                    evaluate(collect(0:(op.deg)), [x], a, b) == res
                    for (a, b) in combinations
            ]
            [
...
@testset "evaluate multivariate basis at point(s)" begin
    for x in X
        ...
        if typeof(x) <: AbstractVector{<:Real}
            [
                @test evaluate(mop.ind, x, a, b) == evaluate(mop.ind, x, mop) ==
                    evaluate(x, mop) ==
            ...
            [
                @test evaluate(x, mop) == evaluate(mop.ind, reshape(x, 1, length(x)), mop) ==
                    evaluate(mop.ind, reshape(x, 1, length(x)), a, b)
                    for (a, b) in combinations
            ]
        else
            [
                @test evaluate(x, mop) == evaluate(mop.ind, x, mop) ==

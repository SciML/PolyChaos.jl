...
@testset "Evaluation of multivariate basis" begin
    for ind in Iterators.product([collect(0:d) for i in 1:n]...)
...
mop = MultiOrthoPoly([GaussOrthoPoly(deg; Nrec = 3 * deg), LogisticOrthoPoly(deg; Nrec = 3 * deg),
                      Uniform01OrthoPoly(deg; Nrec = 3 * deg)], deg)
...
mop = MultiOrthoPoly([GaussOrthoPoly(deg; Nrec = 3 * deg, addQuadrature = false),

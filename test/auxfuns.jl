...
@testset "multivariate integration with MultiOrthoPoly (issue #48)" begin
    ...
    # E[X^2] = 1, E[Y] = 0.5
    @test isapprox(integrate((x, y) -> x^2 * y, mop2), 0.5; atol = 1.0e-10)

    # E[X] * E[Y^2] where E[X] = 0, E[Y^2] = 1/3
    # Result should be 0
    @test isapprox(integrate((x, y) -> x * y^2, mop2), 0.0; atol = 1.0e-14)
    ...
    @test isapprox(mop_result, nested_result; atol = 1.0e-14)
end
